...
use aegislib::crypto::SigningKey;
...
async fn register(config: &Config, key: &SigningKey) -> Result<()> {
    ...
    match register_device(&config.into(), &config.device_name, &key.verifying_key()).await {
        Ok(_) => Ok(()),
